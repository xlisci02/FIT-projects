...
*/
	...
	else s->top=-1; // top = -1 => prazdny zasobnik
}
...
*/
	return ((s->top) == -1); // top = -1 => prazdny zasobnik
}
...
*/
	return (s->top == (STACK_SIZE-1)); // top = STACK_SIZE-1 => plny zasobnik (pole indexovane od 0 do STACK_SIZE-1) 
}
...
** funkci stackEmpty.
*/
	if(stackEmpty(s)) // overenie ci je zas. prazdny
		stackError(SERR_TOP);
	else (*c) = s->arr[s->top]; // vracia vrchol zasobnika
}
...
** 
*/
	if(!(stackEmpty(s))){  // pop nastane iba ak zas. NIEJE prazdny
		(s->top)--; // dekrementacia indexu, ktory ukazuje (znaci) kde je vrchol zasobnika
	}
}
...
** funkci stackFull.
*/
	if(!(stackFull(s))) // vlozi na zasobnik iba ak nieje plny
	{
		s->arr[++s->top] = c; // push na zasobnik + inkrementacia top 
	}	
	else stackError(SERR_PUSH); // chyba pri pokuse o push na plny zasobnik
