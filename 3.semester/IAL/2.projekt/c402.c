...
**/
	int found = FALSE;
	tBTNodePtr where;
	tBTNodePtr tmp = (*RootPtr); // pom. ukazatel na RootPtr
	
	if(!(*RootPtr)){
		found = FALSE; // neplatny uk.
		where=NULL;
	}
	else{
		do{ // hlada sa uzol
	...
	if(!found){ // ak sa nenasiel vytvori sa novy uzol a vlozi ako syn stromu na kt ukazuje where
		...
		new->LPtr=NULL; 
		new->RPtr=NULL;
		if(where == NULL) // pripad prazdneho stromu
			(*RootPtr)=new;
		else{
			if(where->Cont > new->Cont) // vlozi sa do praveho/laveho podstromu
				where->LPtr = new;
			else
				where->RPtr = new;
		}
...
**/
	while(ptr){
		...
		BTWorkOut(ptr); // vypis znaku
		ptr=ptr->LPtr; // prechod do laveho podstromu
	}
}
...
**/
	if(RootPtr){
		tStackP stackP; // init stack
		SInitP(&stackP);
		Leftmost_Preorder(RootPtr, &stackP); // leftmost preorder
		while(!(SEmptyP(&stackP))){ // kym nieje prazdny zasobnik
			tBTNodePtr ptr = STopPopP(&stackP); // ziskan vrchol zasobnika
			Leftmost_Preorder(ptr->RPtr, &stackP); // leftmost preorder pre pravy podstrom
		}
	}
...
**/
	if(RootPtr){
		tStackP stackP; // stack init
		SInitP(&stackP);
		Leftmost_Inorder(RootPtr, &stackP); // leftmost inorder
		while(!(SEmptyP(&stackP))){
			tBTNodePtr ptr = STopPopP(&stackP); // ziska vrchol zasobnika
			BTWorkOut(ptr); // zapise na vystup
			Leftmost_Inorder(ptr->RPtr, &stackP); // leftmost inorder pre pravy podstrom
		}
	}
}
...
**/
	while(ptr){
		SPushP(StackP, ptr); // push na pointers stack  
		SPushB(StackB,true); // push na boolean stack 
		ptr=ptr->LPtr; // prechod do laveho podstromu
	}
}
...
**/
	bool left; // pomocna premenna indikujuca prechod okolo uzla
	tStackP stackP; // stacks init
	SInitP(&stackP);
	tStackB stackB;
	SInitB(&stackB);
	Leftmost_Postorder(RootPtr, &stackP, &stackB); // leftmost postorder
	while(!(SEmptyP(&stackP))){
		tBTNodePtr ptr = STopPopP(&stackP); // ziska vrchol zasbonika pointrov a urobi pop
		SPushP(&stackP,ptr); // zaroven urobi push, celkovo sa urobi akoby iba top
		left = STopPopB(&stackB); // ziska vrchol zasobnika boolean stack
		if(left){
			SPushB(&stackB,false); // idem zlava okolo uzla, pushnem false
			Leftmost_Postorder(ptr->RPtr, &stackP, &stackB); // leftmost postorder pre pravy podstrom
		}
		else{
			STopPopP(&stackP); // idem zprava okolo uzla, tzn.posledny krat
			BTWorkOut(ptr); // zapisem na vystup
		}
	}
...
**/
	//if((*RootPtr)){
		...
		SInitP(&stackP);
		do{
			if(!(*RootPtr)){ // RootPtr == null
				if(!SEmptyP(&stackP)) // ak nieje prazdny stack
					(*RootPtr) = STopPopP(&stackP); // ziskam vrchol zasobnika
			}
			else{
				if((*RootPtr)->RPtr) // ak ma pravy podstrom
					SPushP(&stackP, (*RootPtr)->RPtr); // pushnem ukazatel na pravy podstrom
				tBTNodePtr tmp = (*RootPtr); 
				(*RootPtr) = (*RootPtr)->LPtr; //  prechod do laveho podstromu
				free(tmp); // zrusim otcovsky uzol
			}

		}while((*RootPtr) || !(SEmptyP(&stackP))); // kym niesu zrusene vsetky lave aj prave podstromy
}
