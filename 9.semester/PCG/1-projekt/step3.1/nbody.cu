 ...
 */
 __global__ void centerOfMass(t_particles p, float* comX, float* comY, float* comZ, float* comW, int* lock, const int N)
 {
  ...
  for(int i = 0; i < N; i += blockDim.x * gridDim.x){
    ...
    sh_com_dataX[tid] += p.pos_x[idx] * p.weight[idx];
    sh_com_dataY[tid] += p.pos_y[idx] * p.weight[idx];
    sh_com_dataZ[tid] += p.pos_z[idx] * p.weight[idx];
    sh_com_dataW[tid] += p.weight[idx];
  }
  // Synchronize, every thread in same block should have same shared memory data
  __syncthreads();

  // Reduce, sum of products, same algorithm as in PCG lecture/lab
