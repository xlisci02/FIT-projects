...
int main(int argc, char **argv)
{
  ...
  */
  ...
  cudaStreamCreate(&stream0); // stream for calculate_velocity
  cudaStreamCreate(&stream1); // stream for centerOfMass
  cudaStreamCreate(&stream2); // stream fror cudaMemcopy
  cudaEvent_t VEL_DONE, COM_DONE;
  cudaEventCreate(&VEL_DONE); // calculate_velocity computing is done
  cudaEventCreate(&COM_DONE); // centerOfMass computing is done
  ...
  cudaMemset(com_gpu.x, 0, sizeof(float));
  cudaMemset(com_gpu.y, 0, sizeof(float));
  cudaMemset(com_gpu.z, 0, sizeof(float));
  cudaMemset(com_gpu.w, 0, sizeof(float));
  cudaMemset(lock, 0, 1);
  ...
  for(int s = 0; s < steps; s++)
  {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // stream0
    calculate_velocity<<<simulationGrid, thr_blc, thr_blc*sizeof(float)*7, stream0>>>(p_in, p_out, N, dt);
    cudaEventRecord(VEL_DONE, stream0); // velocity ready for write
    ...
    if (writeFreq > 0 && (s % writeFreq == 0))
    {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // stream1
      centerOfMass<<<simulationGrid, thr_blc, thr_blc * sizeof(float) * 4, stream1>>>(p_in, com_gpu.x, com_gpu.y, com_gpu.z, com_gpu.w, lock, N);
      cudaEventRecord(COM_DONE, stream1);
      cudaStreamWaitEvent(stream1, VEL_DONE, 0);

      // stream2
      // Copy velocity data from GPU to CPU
      cudaMemcpyAsync(particles_cpu.pos_x, p_in.pos_x, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.pos_y, p_in.pos_y, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.pos_z, p_in.pos_z, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.vel_x, p_in.vel_x, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.vel_y, p_in.vel_y, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.vel_z, p_in.vel_z, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(particles_cpu.weight, p_in.weight, N * sizeof(float), cudaMemcpyDeviceToHost, stream2);

      // Write velocity data to file
      // CPU has to wait until the data are on CPU
      cudaStreamSynchronize(stream2);
      h5Helper.writeParticleData(s/writeFreq);


      // Waiting for COM computation
      cudaStreamWaitEvent(stream2, COM_DONE, 0);

      // Copy COM data from GPU to CPU
      cudaMemcpyAsync(com_cpu.x , com_gpu.x, sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(com_cpu.y , com_gpu.y, sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(com_cpu.z , com_gpu.z, sizeof(float), cudaMemcpyDeviceToHost, stream2);
      cudaMemcpyAsync(com_cpu.w , com_gpu.w, sizeof(float), cudaMemcpyDeviceToHost, stream2);

      // Write COM data to file
      // CPU has to wait until the data are on CPU
      cudaStreamSynchronize(stream2);
      ...
      *com_cpu.y /= (*com_cpu.w);
      *com_cpu.z /= (*com_cpu.w);
      h5Helper.writeCom(*com_cpu.x, *com_cpu.y, *com_cpu.z, *com_cpu.w, s/writeFreq);

      // Zero COM data before next computation
      cudaMemsetAsync(com_gpu.x, 0, sizeof(float), stream1);
      cudaMemsetAsync(com_gpu.y, 0, sizeof(float), stream1);
      cudaMemsetAsync(com_gpu.z, 0, sizeof(float), stream1);
      cudaMemsetAsync(com_gpu.w, 0, sizeof(float), stream1);

    }
    // Wait for velocity computation to finish
    cudaStreamWaitEvent(stream2, VEL_DONE, 0);
  ...
  centerOfMass<<<simulationGrid, thr_blc, thr_blc * sizeof(float) * 4>>>(p_in, com_gpu.x, com_gpu.y, com_gpu.z, com_gpu.w, lock, N);
  gettimeofday(&t2, 0);
