...
VertexIndex gpu_computeGLVertexID(
  ...
  if(indices!=NULL)
    return indices[vertexShaderInvocation]; // if indexing is active
  return vertexShaderInvocation;  // otherwise
...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  if(head->enabled) 
  {
    void const * a = (void * )((char *)head->buffer + head->offset + (head->stride * gl_VertexID)); // pointer arithmetic
    return a;
  }
  return NULL; // if head is not enabled return NULL
}
...
void gpu_runVertexPuller(
  ...
  VertexIndex my_VertexID = gpu_computeGLVertexID(puller->indices,vertexShaderInvocation);

  for(int i=0; i < MAX_ATTRIBUTES; i++)
    output->attributes[i] = gpu_computeVertexAttributeDataPointer(&puller->heads[i],my_VertexID);
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput output; // puller output
  GPUVertexShaderInput input; // shader input
  for(unsigned i=0; i < nofPrimitiveVertices; i++)
  { 
    gpu_runVertexPuller(&output,puller,baseVertexShaderInvocation+i); // run Vertex Puller
    input.attributes = &output; 
    input.gl_VertexID = gpu_computeGLVertexID(puller->indices,baseVertexShaderInvocation+i); // get VertexID 
    vertexShader(&primitive->vertices[i],&input,gpu); //  run vertexShader
  }
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  // barycentric coordinate = area of opposite little triagle / area of triangle
  // simpler -> height of opposite little triangle  / height of tiangle 
  // height is distance from vertex/point in tr. to opposite line
